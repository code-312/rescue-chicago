...
breed_trends_tab, other_trends_tab = st.tabs([":blue[📊 Breed Trends by Count]", ":blue[📊 Other Trends by Count]"])
...
with breed_trends_tab:
    ...
    with col1:
        ...
        if len(where_clause) > 0:
            los_by_breed_query = """
                SELECT breed_primary,Count(*) as "%s" FROM "%s" %s %s %s GROUP BY breed_primary %s %s %s;
                """ % (pfglobals.COUNT_TEXT, pfglobals.DATABASE_TABLE, pfglobals.max_los, where_clause, location_clause,  pfglobals.min_animal_count, pfglobals.los_sort, pfglobals.limit_query)
        else:
            los_by_breed_query = """
                SELECT breed_primary,Count(*) as "%s" FROM "%s" %s %s GROUP BY breed_primary %s %s %s;
                """ % (pfglobals.COUNT_TEXT, pfglobals.DATABASE_TABLE, pfglobals.max_los, location_clause, pfglobals.min_animal_count, pfglobals.los_sort, pfglobals.limit_query)
...
with other_trends_tab:
    ...
    st.markdown("Use the location widget in the sidebar to select a location (default is Chicago). "
                "These graphs illustrate how these characteristics impact number of dogs waiting to be adopted for "
                "dogs of all breeds.")
    ...
    los_by_attribute_query = """
        ...
        """ % (selected_list["db_column"], pfglobals.COUNT_TEXT, pfglobals.DATABASE_TABLE, pfglobals.max_los, attr_where_clause, attr_location_clause, selected_list["db_column"], pfglobals.min_animal_count, pfglobals.los_sort, pfglobals.limit_query)
    ...
    #######################################################
        ...
        st.markdown("Select values for other characteristics from the drop down lists to compare on the graphs. "
                "These side-by-side graphs illustrate how these characteristics impact the number of dogs "
                "waiting for adoption for dogs with the specified attributes. ")
